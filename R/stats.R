...
knit_lme <- function(merMod, top_level = 3, digits = 3, aov = TRUE) {
    ...
    if (modsum %?n% 'varcor') {
        ...
        for (i in 1:length(lnames)){
            cat(paste(c('\n\n', rep('#', top_level+1), ' ',paste('Sigma:', lnames[i])), collapse = ''))
            print(knitr::kable(vcov[[i]], digits = digits))
        }
    }
    if (modsum %?n% 'sigma') {
        cat(paste(c('\n\n', rep('#', top_level+1), ' ', 'Sigma: error'), collapse = ''))
        print(knitr::kable(data.frame(Residuals = sqrt(modsum$sigma)), digits = digits))
    }
    if (modsum %?n% 'coefficients') {
        cat(paste(c('\n\n', rep('#', top_level+1), ' ', 'Coefficients'), collapse = ''))
