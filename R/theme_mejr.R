...
theme_mejr <- function(base_size = 11, madj = 0, black_level = 204,
  ...
  ggplot2::update_geom_defaults(
    "text",
    list(size = scale_add(base_size, 0.25), colour = gray_color)
  )
  ...
  ggplot2::theme(
    ...
    title = element_text(
      ...
      size = scale_add(base_size, 1.2), hjust = 0,
    ...
    panel.border = element_rect(
      size = rel(0.9), colour = rgb(0, 0, 0, 0.03)
    ),
...
test_mejr_theme <- function(w = 6.875, h = 4.5, eplot = list(),
  ...
  p <- p + gg
  save_plot(p,
    file = normalizePath(file.path("~/../Desktop/test"),
      mustWork = F), width = w,
    height = h, format = "pdf")
  grid::grid.draw(p)
}
