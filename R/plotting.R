...
#' example_plot()
example_plot <- function(facets = TRUE, ax = 'bottom', ay = 'left',
                         switch = NULL) {
  ...
  p <- ggplot(data = d) + aes(x = carat, y = price) +
    ...
    geom_smooth(
      method = 'lm', se = FALSE, aes(color = clarity, linetype = clarity)) +
...
save_plot <- function(x, file, dir = NULL, width = 5.25, height = 3.8,
                      format = "pdf", font = getOption("mejr.font"),
  ...
  if (any(format %in% c("pdf", "both"))) {
    ...
    dev.off()
    if (!is.null(font)) {
      font <- font_is_registered(font)$embed
      if (font$embed) {
        set_ghostscript_env()
  ...
  if (any(format %in% c("png", "both"))) {
    ...
    png(filename = png_file, width = width, height = height, res = res,
        units = "in")
    lapply(x, plot_switch)
    if (!is.null(fun)) {
      do.call(fun, list(...))
...
#'  heights = c(.4,.4,.2),
#'  widths = c(.6,.4))
combine_plots <- function(..., plots, layout, heights, widths, ncols,
                          show = TRUE) {
  if (missing(plots)) {
    plots <- list(...)
  }
  ...
  if (missing(layout)) {
    ...
    args <- list(
      grobs = plots, layout_matrix = layout, heights = heights,
      widths = widths)
  }
