...
#' @param fps Frames per second of the video source. Defaults to 30 Frames Per Second. The smaller
#'   the value, the more likely two events will be chunked in the same frame.
...
#' @param chunked If set to TRUE, will return a time back to you instead of frame number, but the
#'   chunked/cut value corresponding to that frame.
...
#'
#' If you're going to use a reference date, make sure the format for both dob and ref are the same.
#' For example, don't use ymd for dob and mdy for ref. You'll get wrong values.
#'
#' @param dob Date of birth string that the function \code{\link{ymd}} and others like it can
#'   understand. Typically in the format "yyyy-mm-dd" or "yyyy/mm/dd"
#' @param ref Reference date string. Either today's date or some other time after \code{dob}
#'   Defaults to today.
#' @param lub.fmt Lubridate function for the input dates, such as \code{\link{ymd}} or any function
#'   that returns a \code{POSIXct} format. Defaults to \code{\link{mdy}}
...
#' @param keep_original If set to \code{TRUE} (default), items already existing in \code{arglist}
#'   will not be changed.
#'
#' @return list
#' @export
