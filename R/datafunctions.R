...
stack_csv <- function(folder, files, search = TRUE, ...) {
  ...
  csv_data <- data.table::rbindlist(csv_data, use.names=TRUE, fill=TRUE, idcol = '.csv_file_num')
  classes <- sapply(csv_data, class)

  message('concatendated the following variables:')
  message(paste(paste0("[", 1:length(classes), "]:"), names(classes), "==", as.character(classes), collapse = "\n"))
  return(csv_data)
}
...
#' @param n_chunk_cols if multiple data sets on one sheet, specify number of cols for each subset
#'
#' @return NULL
#' @export
#'
...
list2excel <- function(excel_list, filename, n_chunk_cols=Inf)
{
  ...
  for (i in 1:length(excel_list)) {
    ...
    } else if (isinstance(sheet_data, 'list')) {
      # reset worksheet
      cell_width <- 8
      r_count <- 1
      c_count <- 1
