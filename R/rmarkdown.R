...
generate_doc <- function(filename, format = "html_document", ...) {
  ...
  if (format == "html_document") {
    ...
    yaml_header <- modifyList(
      ...
      code_download = TRUE, theme = "journal",
    ...
    if (format == "pdf_document") {
      ...
      yaml_header <- modifyList(yaml_header, list(dev = "pdf", keep_tex = FALSE))

      # pdf specific knitr options
      r_chunk_opts <- modifyList(
        r_chunk_opts,
        list(knitr = list(opts_chunk = list(out.width = "4.2in",
        out.height = "3.6in"
        )))
      )
    } else {
      if (format == "md_document") {
        ...
        yaml_header <- modifyList(
          yaml_header, list(variant = "markdown", dev = "png",
          highlight = NULL
          )
        )
        r_chunk_opts <- modifyList(
...
update_opts_rmd <- function() {
  ...
  mejr::update_opts(width = non_null(getOption("mejr.width"), 100),
...
show_data <- function(x, nrows = NULL, caption = NULL, print = TRUE, ...) {
  ...
  pan_args <- mejr::append_args(
    list(...), list(round = non_null(getOption("mejr.round"), 2),
...
math_matrix <- function(sym = "", x, rows = NULL, cols = NULL,
  ...
  xtab_txt <- print(xtab, floating = FALSE, tabular.environment = "array",
...
na_slices <- function(x, head = NULL, d = 1) {
  ...
  single_nas <- unlist(sapply(seq_along(rep_na$values),
    function(i) {
...
print_fmt <- function(round = getOption("mejr.round"),
                      digits = getOption("mejr.digits"), format = "g", leading = TRUE) {
...
knit_lme <- function(merMod, top_level = 4, round = getOption("mejr.round"),
  ...
  if (modsum %?n% "coefficients") {
    ...
    cf_tab <- cbind(Effect = rm_colon(rownames(cf_tab)),
    data.table::as.data.table(cf_tab)
    )
    if (class(merMod) == "merModLmerTest") {
      pvals <- cf_tab[, unlist(lapply(names(cf_tab),
        function(i) {
          grepl("Pr", i)
        }
      )), with = F]
  ...
  if (aov) {
    ...
    if (class(merMod) == "merModLmerTest") {
      pvals <- aov_tab[, unlist(lapply(names(aov_tab),
        function(i) {
          grepl("Pr", i)
        }
      )), with = F]
